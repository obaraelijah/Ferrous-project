...
#[derive(Serialize, ToSchema)]
pub struct GetAllDomainsResponse {
...
#[utoipa::path(
    ...
    context_path = "/api/v1",
    responses(
        (status = 200, description = "Retrieve all domains of a workspace", body = GetAllDomainsResponse),
...
pub async fn get_all_domains(
    ...
    session: Session,
    db: Data<Database>,
) -> ApiResult<Json<GetAllDomainsResponse>> {
    ...
    Ok(Json(GetAllDomainsResponse { domains }))
}